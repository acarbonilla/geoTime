...
import { FaExchangeAlt, FaCalendarAlt, FaClock, FaUserCheck, FaUserTimes, FaHourglassHalf, FaSort, FaSortUp, FaSortDown, FaUser, FaCheck, FaTimes } from 'react-icons/fa';
...
const ChangeScheduleApprovalList = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      setError('');
      try {
        const res = await axios.get('change-schedule-requests/');
        setRequests(res.data.results || res.data || []);
      } catch (err) {
        setError('Failed to load change schedule requests.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const submitApproval = async () => {
    ...
    try {
      ...
      await axios.post(`change-schedule-requests/${selectedRequest.id}/${endpoint}/`, {
  ...
  if (loading) return <div className="text-center py-4">Loading change schedule requests...</div>;
  if (error) return <div className="text-red-500 text-center py-4">{error}</div>;

  return (
    <div className="bg-white p-8 rounded-2xl shadow-xl mt-6 animate-fade-in">
      <h2 className="text-2xl font-bold mb-4 text-blue-700 flex items-center gap-2">
        <FaExchangeAlt className="text-blue-400" /> Team Change Schedule Requests
      </h2>
      ...
      {paginatedRequests.length === 0 ? (
        <div className="text-gray-500">No change schedule requests found.</div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-blue-50">
                ...
                <th className="px-3 py-2 text-left">Original Start</th>
                <th className="px-3 py-2 text-left">Original End</th>
                <th className="px-3 py-2 text-left">Requested Date</th>
                <th className="px-3 py-2 text-left">Requested Start</th>
                <th className="px-3 py-2 text-left">Requested End</th>
            ...
            <tbody>
              {paginatedRequests.map((req, idx) => (
                <tr key={req.id} className={`border-b ${idx % 2 === 0 ? 'bg-gray-50' : 'bg-white'} animate-fade-in`} style={{animationDelay: `${idx * 40}ms`}}>
                  ...
                  <td className="px-3 py-2">{req.original_date}</td>
                  <td className="px-3 py-2">{req.original_start_time}</td>
                  <td className="px-3 py-2">{req.original_end_time}</td>
                  <td className="px-3 py-2">{req.requested_date}</td>
                  <td className="px-3 py-2">{req.requested_start_time}</td>
                  <td className="px-3 py-2">{req.requested_end_time}</td>
                  <td className="px-3 py-2 max-w-xs truncate" title={req.reason}>{req.reason || '-'}</td>
                  ...
                  <td className="px-3 py-2 max-w-xs truncate" title={req.comments}>{req.comments || '-'}</td>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50">
            ...
            >
              ...
              <div className="mb-4">
                ...
                <p className="text-gray-600 mb-2">
                  Original: <span className="font-semibold">{selectedRequest?.original_date} {selectedRequest?.original_start_time} - {selectedRequest?.original_end_time}</span>
                </p>
                <p className="text-gray-600 mb-2">
                  Requested: <span className="font-semibold">{selectedRequest?.requested_date} {selectedRequest?.requested_start_time} - {selectedRequest?.requested_end_time}</span>
                </p>
              </div>
              <div className="mb-4">
